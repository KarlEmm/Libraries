...
// TODO: save regrets to a file at intervals in case the program crashes.
...
{
  Card() : rank{0}, suit{0} {};
  explicit Card(uint8_t card)
      : rank{static_cast<uint8_t>(card >> 2)},
        suit{static_cast<uint8_t>(card & 0b11)} {};
  uint8_t rank : 6;
  uint8_t suit : 2;
...
struct Deck
{
  ...
  std::array<Card, Constants::nFlopCards> getFlopCards() const
  {
    return {deck[Constants::communityCardIndex],
            deck[Constants::communityCardIndex + 1],
            deck[Constants::communityCardIndex + 2]};
  }
  std::array<Card, Constants::nTurnCards> getTurnCards() const
  {
    return {deck[Constants::communityCardIndex + 3]};
  }
  std::array<Card, Constants::nRiverCards> getRiverCards() const
  {
    return {deck[Constants::communityCardIndex + 4]};
  }

  std::array<Card, Constants::nPrivateCards> getPreflopHand(int playerIdx) const
  {
    return {deck[playerIdx * 2], deck[playerIdx * 2 + 1]};
  }

  std::array<Card, Constants::nPrivateCards + Constants::nFlopCards>
  getFlopHand(int playerIdx) const
  {
    ...
    std::copy(flopCards.begin(), flopCards.end(),
              flopHand.begin() + Constants::nPrivateCards);

    return flopHand;
  }

  std::array<Card, Constants::nPrivateCards + Constants::nFlopCards +
                       Constants::nTurnCards>
  getTurnHand(int playerIdx) const
  {
    const auto &preflopCards = getPreflopHand(playerIdx);
    const auto &flopCards = getFlopCards();
    ...
    std::copy(flopCards.begin(), flopCards.end(),
              turnHand.begin() + Constants::nPrivateCards);
    std::copy(turnCards.begin(), turnCards.end(),
              turnHand.begin() + Constants::nPrivateCards +
                  Constants::nFlopCards);
  ...
  using RiverHand =
      std::array<Card, Constants::nPrivateCards + Constants::nFlopCards +
                           Constants::nTurnCards + Constants::nRiverCards>;
  RiverHand getRiverHand(int playerIdx) const
  {
    ...
    std::copy(flopCards.begin(), flopCards.end(),
              riverHand.begin() + Constants::nPrivateCards);
    std::copy(turnCards.begin(), turnCards.end(),
              riverHand.begin() + Constants::nPrivateCards +
                  Constants::nFlopCards);
    std::copy(riverCards.begin(), riverCards.end(),
              riverHand.begin() + Constants::nPrivateCards +
                  Constants::nFlopCards + Constants::nRiverCards);
...
{
  std::unique_ptr<
      Histogram<AbstractionsContext::nEHSHistogramsBins>[],
      Memory::MmapDeleter<Histogram<AbstractionsContext::nEHSHistogramsBins>>>
      flopHistogramsMmap;
  std::unique_ptr<
      Histogram<AbstractionsContext::nEHSHistogramsBins>[],
      Memory::MmapDeleter<Histogram<AbstractionsContext::nEHSHistogramsBins>>>
      turnHistogramsMmap;
  std::unique_ptr<
      Histogram<AbstractionsContext::nOCHSHistogramsBins>[],
      Memory::MmapDeleter<Histogram<AbstractionsContext::nOCHSHistogramsBins>>>
      riverHistogramsMmap;
  ...
  std::span<Histogram<AbstractionsContext::nOCHSHistogramsBins>>
      riverHistograms;
...
struct GameContext
{
  ...
  {
    return m_lastBettingPlayer ==
               getBBPlayerIdFromButton(m_currentButtonPlayerId) &&
           m_currentPlayerTurn ==
               getBBPlayerIdFromButton(m_currentButtonPlayerId) &&
...
public:
  struct Key
  {
    // NOTE (keb): 60 bits for at most 5 actions per betting round * 4 betting
    // rounds * 3 bits per action
    // TODO (keb): not flexible, can't add more players
    uint64_t history
        : Constants::maxNActionsPerRound *(uint8_t)BettingRound::RoundCount
          *
          3;
    ...
    {
      return (history == other.history && playerid == other.playerid &&
  ...
  void updateRegrets(const std::array<float, Action::Count> &values,
                     Actions allowedActions, float value)
  {
    ...
    if (nTrainingIterations == 0)
      ++DebugGlobals::nTrained;
    ++nTrainingIterations;
  }
  ...
  void setActions(const GameContext &gameContext)
  {
    ...
    if ((gameContext.m_currentBettingRound == BettingRound::Preflop &&
         gameContext.m_nBetInCurrentRound >= Constants::maxNBetsPreflop) ||
        (gameContext.m_currentBettingRound != BettingRound::Preflop &&
         gameContext.m_nBetInCurrentRound >= Constants::maxNBetsForRound))
    {
      return;
    }
    if (stack > 0)
  ...
  const std::array<float, Action::Count> calculateStrategy()
  {
    ...
    for (int a = 0; a < Action::Count; ++a)
    {
      ...
      if (allowedActions.test(action))
      {
        strategy[a] =
            sum > 0.0
                ? ((regrets[a] <= 0 ? 0 : regrets[a]) / static_cast<float>(sum))
                : (1.0 / nAllowedActions);
      }
    }
...
// TODO (keb): Adjust the total number of possible infosets for your
// abstraction.
// TODO (keb): Find a way to have a smaller key.
ExtHM::ExternalHashMap<__uint128_t, Infoset> infosets("dummySolution.dat",
                                                      1'000'000'000);
...
std::array<AbstractedSizeRatio, Constants::nPlayers>
abstractStacks(int currentPlayerId,
               const std::array<int, Constants::nPlayers> &stacks)
{
  // A player's stack is abstracted to be a multiple of the current player's
  // stack.
...
T getAbstractionIndex(int playerId, const Deck &deck, BettingRound round)
{
  ...
  {
    const auto &hand = deck.getPreflopHand(playerId);
    uint8_t cards[Constants::nCardsRoundAccumulator[(
        uint8_t)BettingRound::Preflop]];
  ...
  case BettingRound::Flop:
  {
    ...
    const auto &hand = deck.getFlopHand(playerId);
    uint8_t
        cards[Constants::nCardsRoundAccumulator[(uint8_t)BettingRound::Flop]];
    std::copy(hand.begin(), hand.end(), cards);
    ...
    result = KMeans::findNearestCentroid<FlopHistogram,
                                         KMeans::L2Distance<FlopHistogram>>(
                 roundAbstractions.flopCentroids, histogram)
                 .first;
    return result;
  }
  case BettingRound::Turn:
  {
    ...
    const auto &hand = deck.getTurnHand(playerId);
    uint8_t
        cards[Constants::nCardsRoundAccumulator[(uint8_t)BettingRound::Turn]];
    std::copy(hand.begin(), hand.end(), cards);
    uint64_t handIndex = hand_index_last(&indexers[(uint8_t)round], cards);
    const auto &histogram = roundAbstractions.turnHistograms[handIndex];
    result = KMeans::findNearestCentroid<TurnHistogram,
                                         KMeans::L2Distance<TurnHistogram>>(
                 roundAbstractions.turnCentroids, histogram)
                 .first;
    return result;
  }
  case BettingRound::River:
  {
    ...
    const auto &hand = deck.getRiverHand(playerId);
    uint8_t
        cards[Constants::nCardsRoundAccumulator[(uint8_t)BettingRound::River]];
    std::copy(hand.begin(), hand.end(), cards);
    uint64_t handIndex = hand_index_last(&indexers[(uint8_t)round], cards);
    const auto &histogram = roundAbstractions.riverHistograms[handIndex];
    result = KMeans::findNearestCentroid<RiverHistogram,
                                         KMeans::L2Distance<RiverHistogram>>(
                 roundAbstractions.riverCentroids, histogram)
                 .first;
    return result;
  }
  default:
...
uint64_t serializeHistory(const GameHistory &history)
{
  ...
  for (int i = 0; i < (uint8_t)BettingRound::RoundCount; ++i)
  {
    for (int j = 0; j < Constants::maxNActionsPerRound; ++j)
    {
      ...
      result |= (uint64_t)a
                << ((i * Constants::maxNActionsPerRound * 3) + (j * 3));
    }
  }
  return result;
}
...
//       actions/actions/actions/actions, where the '/' seperates the betting
//       rounds. An information set key is an encoding of the playerId, the
//       information abstraction index, the game history, the players' stacks
//       relative to the current state player, the pot size relative to the
...
Infoset::Key
makeInfosetKey(const GameHistory &history, int playerId, const Deck &deck,
               BettingRound bettingRound,
               std::array<AbstractedSizeRatio, Constants::nPlayers> stacks,
               AbstractedSizeRatio pot, FoldedPlayers foldedPlayers)
{
  // TODO (keb): review this
  Infoset::Key key;
  uint8_t clusterIndex =
      getAbstractionIndex<uint8_t>(playerId, deck, bettingRound);
  assert(clusterIndex < AbstractionsContext::maxNBuckets &&
         "Invalid Information Abstraction Cluster Index");
  key.clusterIndex = clusterIndex;
  key.playerid = playerId;
  key.history = serializeHistory(history);

  // NOTE (keb): Uncomment if you want to consider the relative stack and pot
  // sizes. key.stacks = 0; for (int i = 0; i < Constants::nPlayers; ++i)
  // {
  // 	bool isFolded = foldedPlayers.test(i);
  // 	// NOTE (keb): shift bits by 3 because we reserve 3 bits to represent a
  // stack. 	key.stacks |= (isFolded ? ((uint8_t)Zero << (i*3)) :
  // (uint8_t(stacks[i]) << (i*3)));
  // }
  // key.pot = (uint8_t)pot;
...
Action sampleAction(double rand,
                    const std::array<float, Action::Count> &strategy,
                    Actions allowedActions)
{
  float randAcc = 0.0;
  for (int i = 0; i < strategy.size(); ++i)
  {
    // TODO (keb): can get rid of this branch. An illegal action will have a
    // corresponding strategy entry equal to zero.
...
void call(GameContext &gameContext)
{
  ...
  int amount =
      gameContext.m_currentBet > stack ? stack : gameContext.m_currentBet;
  stack -= amount;
  gameContext.m_pot += amount;
  gameContext.m_moneyInPot[gameContext.m_currentPlayerTurn] += amount;
}

void undoCall(GameContext &gameContext, int previousStack, int previousBet,
              int previousPot)
{
  auto &stack = gameContext.m_stacks[gameContext.m_currentPlayerTurn];
...
void undoBet(int playerId, int amount, GameContext &gameContext,
             int previousBet, int previousPot, int previousLastBettingPlayer)
{
  gameContext.m_moneyInPot[playerId] -= amount;
  gameContext.m_stacks[playerId] += amount;
  gameContext.m_pot -= amount;
...
void undoBetRelativePot(GameContext &gameContext, double ratio,
                        int previousStack, int previousBet, int previousPot,
                        int previousLastBettingPlayer)
{
  int amount = previousStack >= ratio * previousPot
                   ? static_cast<int>(ratio * previousPot)
                   : previousStack;
  undoBet(gameContext.m_currentPlayerTurn, amount, gameContext, previousBet,
          previousPot, previousLastBettingPlayer);
}
...
void unApplyAction(GameContext &gameContext, Action a, int previousStack,
                   int previousBet, int previousPot,
                   int previousLastBettingPlayer)
{
  switch (a)
  {
  ...
  case BetOver:
  {
    undoBetRelativePot(gameContext, toDouble(a), previousStack, previousBet,
                       previousPot, previousLastBettingPlayer);
    break;
  }
  default:
...
void snapBettingRound(const GameContext &gameContext,
...
void undoBettingRound(const PreviousBettingRoundSnapshot &snapshot,
...
std::optional<float> isTerminalState(GameContext &gameContext)
{
  ...
  // River Showdown
  if ((gameContext.m_currentPlayerTurn == gameContext.m_lastBettingPlayer &&
       !(gameContext.isBBPreflopException())) &&
      gameContext.m_currentBettingRound == BettingRound::River)
  {
    ...
    for (int i = 0; i < Constants::nPlayers; ++i)
    {
      ...
      if (int handStrength = std::apply(evaluate_7cards, hand);
          handStrength <= bestHandSeen) // smaller index is stronger
      {
    ...
    if (tyingPlayerIds.find(gameContext.m_updatingPlayerId) !=
        tyingPlayerIds.end())
    {
      return static_cast<int>(
          gameContext.m_pot /
          tyingPlayerIds.size()); // Integer Division, inaccuracy negligible
    }
    else
    {
      return -gameContext.m_moneyInPot[gameContext.m_updatingPlayerId];
...
PreviousBettingRoundSnapshot moveToNextRound(GameContext &gameContext)
{
  ...
  if ((gameContext.m_currentPlayerTurn == gameContext.m_lastBettingPlayer &&
       !(gameContext.isBBPreflopException())) ||
      gameContext.m_isCheckAfterBBPreflopException || gameContext.allinState())
  {
    gameContext.m_isCheckAfterBBPreflopException = false;

    snapBettingRound(gameContext, snapshot);
    // Move to the next betting round
    gameContext.m_currentBettingRound =
        (BettingRound)((uint8_t)gameContext.m_currentBettingRound + 1);
    gameContext.m_currentPlayerTurn = getSBPlayerIdFromButton(
        gameContext.m_currentButtonPlayerId); // The SB starts the new round.
...
float traverseMccfrWithNewAction(Action a, GameContext &gameContext,
                                 float &actionValue, float strategy,
                                 bool isPruning)
{
  float result = 0.0f;
  gameContext.m_history[(uint8_t)gameContext.m_currentBettingRound]
                       [gameContext.m_roundActionIndex] = a;
  ...
  gameContext.m_currentPlayerTurn =
      getNextPlayer(gameContext.m_currentPlayerTurn);
  actionValue = traverseMccfr(gameContext, isPruning);
  gameContext.m_currentPlayerTurn =
      getPreviousPlayer(gameContext.m_currentPlayerTurn);
  unApplyAction(gameContext, a, previousStack, previousBet, previousPot,
                previousLastBettingPlayer);
  result = strategy * actionValue;
  --gameContext.m_roundActionIndex;
  gameContext.m_history[(uint8_t)gameContext.m_currentBettingRound]
                       [gameContext.m_roundActionIndex] = Action::Count;
  return result;
}
...
float traverseMccfr(GameContext &gameContext, const bool isPruning)
{
  ...
  // Fold or Allin
  else if (gameContext.m_foldedPlayersStatus.test(
               gameContext.m_currentPlayerTurn) ||
           gameContext.m_stacks[gameContext.m_currentPlayerTurn] <= 0)
  {
    gameContext.m_currentPlayerTurn =
        getNextPlayer(gameContext.m_currentPlayerTurn);
    value = traverseMccfr(gameContext, isPruning);
  ...
  {
    auto abstractedStacks =
        abstractStacks(gameContext.m_currentPlayerTurn, gameContext.m_stacks);
    auto abstractedPot =
        abstractPot(gameContext.m_pot,
                    gameContext.m_stacks[gameContext.m_currentPlayerTurn]);
    Infoset::Key infosetKey = makeInfosetKey(
        gameContext.m_history, gameContext.m_currentPlayerTurn,
        gameContext.m_deck, gameContext.m_currentBettingRound, abstractedStacks,
        abstractedPot, gameContext.m_foldedPlayersStatus);
    auto [infosetWrapped, isNewState] =
        infosets.insert(infosetKey.asInteger(), Infoset());
    ...
    if (gameContext.m_currentPlayerTurn == gameContext.m_updatingPlayerId)
    {
      ...
      for (int i = 0; i < Action::Count; ++i)
      {
        ...
        if (isPruning &&
            infoset.getRegrets()[a] < Constants::pruningRegretThreshold)
        {
          allowedActions.unset(a);
          continue;
        }
        value += traverseMccfrWithNewAction(a, gameContext, actionValues[i],
                                            strategy[i], isPruning);
      }
...
void initAbstractions()
{
  ...
  const auto nFlopSize = hand_indexer_size(
      &indexers[(uint8_t)BettingRound::River], (uint8_t)BettingRound::Flop);
  const auto nTurnSize = hand_indexer_size(
      &indexers[(uint8_t)BettingRound::River], (uint8_t)BettingRound::Turn);
  const auto nRiverSize = hand_indexer_size(
      &indexers[(uint8_t)BettingRound::River], (uint8_t)BettingRound::River);

  roundAbstractions.flopHistogramsMmap =
      Memory::getMmap<Histogram<nEHSHistogramsBins>>(
          (std::string("../EHS/") + flopHistogramsFilename).c_str(), nFlopSize);
  roundAbstractions.turnHistogramsMmap =
      Memory::getMmap<Histogram<nEHSHistogramsBins>>(
          (std::string("../EHS/") + turnHistogramsFilename).c_str(), nTurnSize);
  roundAbstractions.riverHistogramsMmap =
      Memory::getMmap<Histogram<nOCHSHistogramsBins>>(
          (std::string("../EHS/") + riverOCHSFilename).c_str(), nRiverSize);

  roundAbstractions.flopCentroidsMmap =
      Memory::getMmap<Histogram<nEHSHistogramsBins>>(
          (std::string("../EHS/") + flopCentroidsFilename).c_str(),
          nFlopBuckets);
  roundAbstractions.turnCentroidsMmap =
      Memory::getMmap<Histogram<nEHSHistogramsBins>>(
          (std::string("../EHS/") + turnCentroidsFilename).c_str(),
          nTurnBuckets);
  roundAbstractions.riverCentroidsMmap =
      Memory::getMmap<Histogram<nOCHSHistogramsBins>>(
          (std::string("../EHS/") + riverCentroidsFilename).c_str(),
          nRiverBuckets);

  roundAbstractions.flopHistograms = {
      roundAbstractions.flopHistogramsMmap.get(),
      roundAbstractions.flopHistogramsMmap.get() + nFlopSize};
  roundAbstractions.turnHistograms = {
      roundAbstractions.turnHistogramsMmap.get(),
      roundAbstractions.turnHistogramsMmap.get() + nTurnSize};
  roundAbstractions.riverHistograms = {
      roundAbstractions.riverHistogramsMmap.get(),
      roundAbstractions.riverHistogramsMmap.get() + nRiverSize};

  roundAbstractions.flopCentroids = {roundAbstractions.flopCentroidsMmap.get(),
                                     roundAbstractions.flopCentroidsMmap.get() +
                                         nFlopBuckets};
  roundAbstractions.turnCentroids = {roundAbstractions.turnCentroidsMmap.get(),
                                     roundAbstractions.turnCentroidsMmap.get() +
                                         nTurnBuckets};
  roundAbstractions.riverCentroids = {
      roundAbstractions.riverCentroidsMmap.get(),
      roundAbstractions.riverCentroidsMmap.get() + nRiverBuckets};
}

void placeBlinds(GameContext &gameContext)
{
  bet(getSBPlayerIdFromButton(gameContext.m_currentButtonPlayerId),
      Constants::sb, gameContext);
  bet(getBBPlayerIdFromButton(gameContext.m_currentButtonPlayerId),
      Constants::bb, gameContext);
...
void updateEachPlayer(GameContext &gameContext, bool isPruningIteration)
{
  for (int playerId = 0; playerId < Constants::nPlayers; ++playerId)
  {
    ...
    gameContext.m_currentPlayerTurn = getNextPlayer(
        getBBPlayerIdFromButton(gameContext.m_currentButtonPlayerId));
...
{
  // TODO (keb): Take a snapshot of the current strategy to make an average of
  // the snapshots later. for (player in players) if (t %
  // Constants::strategyInterval == 0)
  // {
...
void discountRegrets(int timeElapsed)
{
      ...
      (timeElapsed % Constants::discountIntervalMinutes == 0))
  {
    double discount =
        (timeElapsed /
         static_cast<double>(Constants::discountIntervalMinutes)) /
        ((timeElapsed /
          static_cast<double>(Constants::discountIntervalMinutes)) +
         1);
